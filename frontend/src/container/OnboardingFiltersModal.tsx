...
export function OnboardingFiltersModal({
  ...
  // Get group options for a specific class
  const getGroupsForClass = (classId: number) => {
    // Find the mapping for this class
    const mapping = classGroupMappings.find((m) => m.classId === classId);
    if (!mapping) return [];

    // Return only the groups that are in this class's mapping
    const allowedGroupIds = new Set(mapping.groupIds);
    return groups
      .filter((g) => {
  ...
  const selectAllFor = (classId: number) => {
    const groupsForClass = getGroupsForClass(classId);
    setSel({...sel, [classId]: groupsForClass.map((g) => g.id)});
  };
      ...
      <div className='space-y-4'>
        ...
        <div className='space-y-3'>
          {filteredClasses.map((c) => {
            const groupsForClass = getGroupsForClass(c.id);
            const selected = new Set(sel[c.id] ?? []);
            const count = selected.size;
            const all = count === groupsForClass.length && count > 0;
            return (
              <div
                key={c.id}
